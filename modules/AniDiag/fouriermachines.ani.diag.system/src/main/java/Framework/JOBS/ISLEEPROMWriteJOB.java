...
public class ISLEEPROMWriteJOB extends Job implements KineticsResponseConvey
{
...
//End KineticsResponseConvey delegate
    ...
    public ISLEEPROMWriteJOB(EEPROMDetails Details, ISLWriteConvey delegate, ArrayList<Integer> Data) {
        super();
        ...
        switch(CURRENT_STATE) {
            ...
            case WRITE_REQUEST: {
                if (KineticComms.Instance.IsConnectedToMachine()) {
                    ...
                    WaitingForKineticsRequestAck = KineticComms.Instance.WriteToISLEEPROM(WriteDetails.Address, 1 ,DataBytes.get(Index));
                } else {
            ...
            break;
            case CHECK_RESPONSE: {
                if(KineticComms.Instance.CheckWriteToISLEEPROM(LastKineticsResponse.get(0))) {
                    Index++;
