...
public class EEPROMReadJOB extends Job implements KineticsResponseConvey
{


    EEPROMDetails ReadDetails;
    ArrayList<Integer> DataBytes;
    EEPromReadConvey ReadConvey;
...
//End KineticsResponseConvey delegate
    ...
    public  EEPROMReadJOB(EEPROMDetails Details, EEPromReadConvey delegate) {
        super();
        ...
        switch(CURRENT_STATE) {
            ...
            case READ_REQUEST: {
                if (KineticComms.Instance.IsConnectedToMachine()) {
                    ...
                    WaitingForKineticsRequestAck = KineticComms.Instance.ReadFromEEPROM(ReadDetails);
                } else {
            ...
            break;
            case EXTRACT_BYTES: {
                DataBytes = KineticComms.Instance.ExtractBytesFromEEPROMReadResponse(LastKineticsResponse.get(0));
                ...
                break;
            case FINISH: {
                ReadConvey.EEPROMReadRecieved(ReadDetails, DataBytes);
                Pause();
            }
                break;
            default:
