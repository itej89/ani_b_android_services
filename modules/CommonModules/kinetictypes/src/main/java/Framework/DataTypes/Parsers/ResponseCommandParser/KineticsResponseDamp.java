...
//DMP#ADR#K#O:
public class KineticsResponseDamp extends KineticsResponse
{
     Actuator ActuatorType;
    Integer Damping;
    
    public KineticsResponseDamp(String response) {
        super(response);
        if(ResponseType == CommandLabels.CommandTypes.FRQ)
        {
            ...
            if(RequestAckParts.size() == 4)
            {
                ...
                Damping = Integer.parseInt(RequestAckParts.get(2));
    ...
    protected KineticsResponseDamp(Parcel in) {
        super(in);
    ...
    public static final Creator<KineticsResponseDamp> CREATOR = new Creator<KineticsResponseDamp>() {
        @Override
        public KineticsResponseDamp createFromParcel(Parcel source) {
            return new KineticsResponseDamp(source);
        }

        @Override
        public KineticsResponseDamp[] newArray(int size) {
            return new KineticsResponseDamp[size];
        }
    };
}
